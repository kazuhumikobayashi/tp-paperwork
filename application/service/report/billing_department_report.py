...
class BillingDepartmentReport(object):
    ...
    def _create_excel_department(self):
        ...
        for i, project_month in enumerate(self.project_months):
            if i != 0:
                ...
                if company_name != project_month.project.client_company.company_name:
                    self.amount_total(i, 'D', company_name, confirmation_total_company,
                ...
                if department_name != project_month.project.recorded_department.department_name:
                    self.amount_total(i, 'C', department_name, confirmation_total_department,
                ...
                if group_name != project_month.project.recorded_department.group_name:
                    self.amount_total(i, 'B', group_name, confirmation_total_group,
            ...
            self.ws['H' + str(i + self.current_row)].value = project_month.billing_confirmation_money or 0
            self.ws['I' + str(i + self.current_row)].value = project_month.tax_of_billing_confirmation_money()
            self.ws['J' + str(i + self.current_row)].value = (project_month.billing_confirmation_money or 0) \
                + project_month.tax_of_billing_confirmation_money()
            self.ws['K' + str(i + self.current_row)].value = project_month.deposit_date
            self.ws['L' + str(i + self.current_row)].value = project_month.project.client_company.bank.bank_name
            # 金額セルの書式設定
            self.money_format(i, 'department')
            # 日付セルの書式設定
            self.day_format(i, 'department')
            # "顧客"単位の各合計金額
            confirmation_total_company += project_month.billing_confirmation_money or 0
            ...
            amount_total_department += (project_month.billing_confirmation_money or 0) \
                + project_month.tax_of_billing_confirmation_money()
            # "本部"単位の各合計金額
            confirmation_total_group += project_month.billing_confirmation_money or 0
        ...
        # 値を代入
        self.ws['H' + str(i + self.current_row + 1)].value = confirmation_total_company
        self.ws['I' + str(i + self.current_row + 1)].value = tax_total_company
        self.ws['J' + str(i + self.current_row + 1)].value = amount_total_company
        self.ws['H' + str(i + self.current_row + 2)].value = confirmation_total_department
        self.ws['I' + str(i + self.current_row + 2)].value = tax_total_department
        self.ws['J' + str(i + self.current_row + 2)].value = amount_total_department
        self.ws['H' + str(i + self.current_row + 3)].value = confirmation_total_group
        self.ws['I' + str(i + self.current_row + 3)].value = tax_total_group
        self.ws['J' + str(i + self.current_row + 3)].value = amount_total_group
        self.ws['H' + str(i + self.current_row + 4)].value = confirmation_total_all
        self.ws['I' + str(i + self.current_row + 4)].value = tax_total_all
        self.ws['J' + str(i + self.current_row + 4)].value = amount_total_all
        # セルの結合
    ...
    def _create_excel_deposit(self):
        ...
        for i, project_month in enumerate(self.project_months):
            if i != 0:
                if company_name != project_month.project.client_company.company_name:
                        ...
                        # 値を代入
                        self.ws['C' + str(i + self.current_row)].value = confirmation_total_department
                        self.ws['D' + str(i + self.current_row)].value = tax_total_department
                        self.ws['E' + str(i + self.current_row)].value = amount_total_department
                        self.ws['F' + str(i + self.current_row)].value = deposit_date
                        self.ws['G' + str(i + self.current_row)].value = bank_name
                        # 金額セルの書式設定
            ...
            self.ws['C' + str(i + self.current_row)].value = project_month.billing_confirmation_money or 0
            self.ws['D' + str(i + self.current_row)].value = project_month.tax_of_billing_confirmation_money()
            self.ws['E' + str(i + self.current_row)].value = (project_month.billing_confirmation_money or 0) \
                + project_month.tax_of_billing_confirmation_money()
            self.ws['F' + str(i + self.current_row)].value = project_month.deposit_date
            self.ws['G' + str(i + self.current_row)].value = project_month.project.client_company.bank.bank_name
            # セルの書式設定
            self.create_outline(i, 'deposit')
        ...
        # 値を代入
        self.ws['C' + str(i + self.current_row + 1)].value = confirmation_total_department
        self.ws['D' + str(i + self.current_row + 1)].value = tax_total_department
        self.ws['E' + str(i + self.current_row + 1)].value = amount_total_department
        self.ws['F' + str(i + self.current_row + 1)].value = deposit_date
        self.ws['G' + str(i + self.current_row + 1)].value = bank_name
        self.ws['C' + str(i + self.current_row + 2)].value = confirmation_total_all
        self.ws['D' + str(i + self.current_row + 2)].value = tax_total_all
        self.ws['E' + str(i + self.current_row + 2)].value = amount_total_all
        for j in range(3):
            # セルの書式設定
    ...
    def day_format(self, index, kind):
        ...
        if kind == self.DEPARTMENT:
            ...
            self.ws['F' + str(index + self.current_row)].number_format = 'yyyy/mm/dd'
    ...
    def amount_total(self, index, col, name, confirmation, tax_total, amount_total):

        self.ws[col + str(index + self.current_row)].value = name + "合計"
        # 値を代入
        self.ws['H' + str(index + self.current_row)].value = confirmation
        self.ws['I' + str(index + self.current_row)].value = tax_total
        self.ws['J' + str(index + self.current_row)].value = amount_total
        # 金額セルの書式設定
