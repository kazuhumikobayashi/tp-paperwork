...
class CompanyForm(FlaskForm):
    ...
    company_name_abbreviated = StringField('会社略称', [Length(max=32)], filters=[lambda x: x or None])
    client_flag = SelectMultipleField('顧客フラグ（必須）', [DataRequired(), Length(max=2048), validators.optional()], coerce=int)
    ...
    address1 = StringField('住所', [Length(max=1024)], filters=[lambda x: x or None])
    ...
    def validate_payment_site(self, field):
        if ClientFlag.CLIENT.value in self.client_flag.data and self.payment_site.data is None:
            raise ValidationError('顧客の場合、入力必須です。')
    
    def validate_receipt_site(self, field):
        if ClientFlag.BP.value in self.client_flag.data and self.receipt_site.data is None:
