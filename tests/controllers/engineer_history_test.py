...
class EngineerHistoryTests(BaseTestCase):
    ...
    def test_save_engineer_history(self):
        ...
        engineer_history_after_save = self.engineer_history_repository.find_by_id(
            engineer_history_id)
        actual = engineer_history_after_save.receipt_per_month
    ...
    def test_validation_receipt_start_day1(self):
        # ログインする
        self.app.post('/login', data={
            'shain_number': 'test1',
            'password': 'test'
        ...
        before_engineer_history = self.engineer_history_repository.find_by_engineer_id(
            engineer_id)[0]
        before_receipt_start_day = before_engineer_history.receipt_start_day

        # 前回の支払い終了年月が2017年5月のため、開始年月をその前の年月で登録できないことを確認する。
        result = self.app.post('/engineer_history/' + str(before_engineer_history.id), data={
            'receipt_start_day': date(2017, 3, 1).strftime('%Y/%m'),
            'receipt_end_day': before_engineer_history.receipt_end_day,
            'receipt_per_month': before_engineer_history.receipt_per_month,
            'receipt_rule': before_engineer_history.receipt_rule,
            'receipt_fraction_calculation1': '',
            'receipt_fraction_calculation2': ''
        })
        self.assertEqual(result.status_code, 200)
        ...
        after_engineer_history = self.engineer_history_repository.find_by_engineer_id(
            engineer_id)[0]
        after_receipt_start_day = after_engineer_history.receipt_start_day

        # 更新前と更新後の契約開始年月に変化がないことを確認。
        self.assertEqual(before_receipt_start_day, after_receipt_start_day)
    ...
    def test_validation_receipt_start_day2(self):
        # ログインする
        self.app.post('/login', data={
            'shain_number': 'test1',
            'password': 'test'
        ...
        result = self.app.post('/engineer_history/' + str(before_engineer_history.id), data={
            'receipt_start_day': date(2017, 10, 1).strftime('%Y/%m'),
            'receipt_end_day': date(2017, 8, 1).strftime('%Y/%m'),
            'receipt_per_month': before_engineer_history.receipt_per_month,
            'receipt_rule': before_engineer_history.receipt_rule,
            'receipt_fraction_calculation1': '',
            'receipt_fraction_calculation2': ''
    ...
    def test_through_free_hour_validation(self):
        ...
        result = self.app.post('/engineer_history/create?engineer_id=' + str(engineer_id), data={
            ...
            'receipt_per_bottom_hour': 3,
            'receipt_per_top_hour': 4,
